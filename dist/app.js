...
// 饮品模型（数据源：wiki_tags.饮品，从维基 Data namespace 提取）
...
function _buildDrinks(){
  if(!WT_DRINK) return {containers:[], cups:new Set(), cupAsp:{}, drinkBase:new Map(), drinkGroups:[]};
  // 容器：大对象存项目名；杯/后继 保留维基原名（内部查表用）
  const containers = [];
  for(const id in WT_DRINK.容器){
    const r = WT_DRINK.容器[id];
    containers.push({名称:DRINK_WIKI2PROJ[r.name]||r.name,
                     小杯:r.杯||null, 后继:r.后继||null,
                     比例:r.比例||1, 含酒精:!!r.含酒精, 准则:r.准则||{}, id});
  }
  // 杯子：set/asp 存项目名；drinkBase key 用维基原名（匹配容器的「杯」）
  const cups = new Set(), cupAsp = {};
  for(const id in WT_DRINK.杯子){
    const r = WT_DRINK.杯子[id];
    const nm = DRINK_WIKI2PROJ[r.name]||r.name;
    cups.add(nm); cupAsp[nm] = r.准则||{};
  }
  const drinkBase = new Map();
  containers.forEach(c => {
    if(c.小杯){
      if(!drinkBase.has(c.小杯)) drinkBase.set(c.小杯, []);
...
const DRINK_BIG = _DR.containers, DRINK_SMALL_ARR = Array.from(_DR.cups),
  DRINK_BASE = _DR.drinkBase, DRINK_CUPS = _DR.cups,
  DRINK_GROUPDATA = _DR.drinkGroups, CUP_ASP = _DR.cupAsp;
const INGREDIENTS = CAT.食材清单 || [], COMPANIES = CAT.公司 || [];
...
const ING_SET = new Set(INGREDIENTS.map(x => x.名称));

// ---- 维基标签体系（wiki_tags）判定层 ----
// 来源：维基 Data:*.json 的 aspects + inherits 合并判定（见 tools/extract_food.py）。
// 规则：inherits=_sustenance/_beverage → 食物；inherits=_ingredient → 食材；
//       只有食材没有食物 → 不算菜；有 raw → 不能上桌；有 course.* → 正式菜。
const WT = DATA.wiki_tags || {};
// 维基「可食用原材料」按中文名索引
const WT_RAW = Object.fromEntries(Object.entries(WT.原材料 || {}).map(([k, v]) => [k, v]));
// 维基正式菜按中文名索引
const WT_DISH = Object.fromEntries(Object.entries(WT.菜 || {}).map(([k, v]) => [k, v]));
// 例外的可生吃项（用户确认不按维基判定，保留旧行为）
const RAW_EAT_OVERRIDE = new Set(Object.keys(WT.例外_可生吃 || {}));
// 可生吃上桌的名单 = 维基可食用原材料 ∪ 例外项。
// 正式菜（菜）不在此列——它们走菜谱 canCook 判定，不在此处重复登记。
const RAW_EATABLE = new Set([...Object.keys(WT_RAW), ...RAW_EAT_OVERRIDE]);   // 食材名集合（区分公司商品中的非食材：金属丝、胶水、纸、油等）
...
// 属性值转义：全页 17 处 onclick 都是 onclick="fn('...')" 的形式——属性用 " 包裹、
// 内层 JS 用 ' 包裹，所以单引号也必须转义，否则数据里出现撇号（如 "Don't 派"）
// 会截断 onclick 字符串、按钮静默失效。querySelector('[data-detail="..."]') 读取时
// 浏览器会先解码属性值，&#39; 会变回 '，匹配不受影响。
...
function bigOfDrinks(name){
  // 从维基分装链（DRINK_BASE）查：杯名 -> 能产出该杯的所有容器
  return DRINK_BASE.get(name) || [];
}
...
function dish(n){
  ...
  if(g){
    ...
    let cat = '食材';
    let cat2 = null;
    // 分类来自维基 wiki_tags（食材清单上的 分类列表 字段）
    const wcats = g.分类列表;
    if(Array.isArray(wcats) && wcats.length){
      cat = wcats[0];
      if(wcats.length >= 2) cat2 = wcats[1];
    }
    const obj = { 名称:n, 分类:cat, 产物性相:g.性相||null, 加工步数:0,
      可获取来源:(g.来源||[]).join('、'), 原始:1, 性相来源:g.性相来源,
      raw_eatable: RAW_EATABLE.has(n) };
...
function allKnownItems(){
  ...
  DRINK_BIG.forEach(b=>s.add(b.名称));
  DRINK_SMALL_ARR.forEach(x=>s.add(x));
...
function canServe(f){
  ...
  return canCook(f).ok || RAW_EATABLE.has(f.名称);
}
...
function doEat(name){
  if(!RAW_EATABLE.has(name)){ alert('不可生吃：'+name); return; }
...
function stockItems(){
  ...
  // 饮品分类登记（数据源 wiki_tags.饮品.类别，名字用维基中文名）
  DRINK_GROUPDATA.forEach(g => {
    if(g.items.length) cats.push({title: g.title, items: g.items});
  });
  return cats;
}
...
// 饮品小杯的性相：容器分装的杯取容器准则；直接上架的杯取维基杯子自己的准则
function aspForDrink(name){
  const big=bigOfDrink(name);
  if(big && big.准则 && Object.keys(big.准则).length) return big.准则;
  if(CUP_ASP) return CUP_ASP[name] || null;
  return null;
}
...
function renderDrinks(){
  ...
  // 饮品小杯上架列表
  const smalls = DRINK_SMALL_ARR.slice().sort();
...
function renderMenu(){
  ...
  // RAW_EATABLE 是 Set，需 Array.from 转数组才能 filter/map
  const pool = snapshot();
  const rawItems = Array.from(RAW_EATABLE)
    .filter(n => (pool[n]||0) > 0)
  ...
  if(!items.length){
    ...
    items.forEach(x=>{
      ...
      const isInt = isIntermediate(f.名称) && !RAW_EATABLE.has(f.名称);
      const can = canCook(f).ok;
      const rawFlag = RAW_EATABLE.has(f.名称);
